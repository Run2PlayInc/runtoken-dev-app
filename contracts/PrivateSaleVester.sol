...
/**
 * @title VestingStandardToken
 * @dev VestingStandardToken is a contract for managing vesting of StandardToken Token.
 * @dev The tokens are unlocked continuously to the vester.
 * @dev The contract host the tokens that are locked for the vester.
 */
contract PrivateSaleVester {
    ...
    event NewGrant(address from, address to, address token, uint256 vestedAmount, uint256 startTime, uint256 cliffTime, uint256 endTime);
     ...
     *
     * @param _token The StandardToken token contract address.
    ...
    public
    {
        ...
        require(StandardToken(_token).transfer(msg.sender, amountToSend));
    }
